...
async function initGraebnerSearch() {
  const input = document.querySelector("[data-graebner-search]");
  const results = document.querySelector("[data-graebner-search-results]");
  const status = document.querySelector("[data-graebner-search-status]");
  if (!input || !results) return;

  let searchIndex = [];

  try {
    searchIndex = await fetch("/assets/graebner/search-index.json").then((response) => response.json());
  } catch {
    results.innerHTML = `<p class="search-empty">The Graebner search index could not be loaded right now.</p>`;
    if (status) status.textContent = "Search is temporarily unavailable.";
    return;
  }

  if (status) {
    status.textContent = `Search ${searchIndex.length} local Graebner pages by title or text.`;
  }
  ...
  input.addEventListener("input", () => {
    ...
    if (!matches.length) {
      results.innerHTML = `<p class="search-empty">No matching Graebner pages were found.</p>`;
...
function buildSnippet(entry, query, terms) {
  ...
  if (!text) {
    return summary || "Open this page in A. L. Graebner's Outlines of Doctrinal Theology.";
  }
